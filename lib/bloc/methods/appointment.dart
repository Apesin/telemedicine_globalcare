...
Future<GResponse> performFixAppointment(payload) async {
  final Completer<bool> statusCompleter = Completer();
  Map<String, dynamic> extraData = <String, dynamic>{};
  String message = "";

  try {
    Response? response = await PSSAHttp.post('$APPOINTMENT', payload);
...
Future<GResponse> performGetFixedAppointments(payload) async {
  final Completer<bool> statusCompleter = Completer();
  Map<String, dynamic> extraData = <String, dynamic>{};
  String message = "";

  try {
    Response? response = await PSSAHttp.get('$APPOINTMENTS$payload');
    print(response?.statusCode);
    print(response?.statusMessage);
    if (((response?.statusCode ?? 200) >= 200 && (response?.statusCode ?? 200) <= 300)) {
      print(response?.data);
